-- CREATE CHAT TABLES (SIMPLIFIED - NO ENROLLMENT DEPENDENCY)
...
-- ============================================
-- COURSE CHAT POLICIES (SIMPLIFIED)
-- ============================================

-- Policy: Authenticated users can view course chat messages
CREATE POLICY "Authenticated users can view course chat messages"
    ...
    USING (
        ...
        AND (
            ...
            OR
            -- Allow all authenticated users (can be restricted later)
            auth.uid() IS NOT NULL
        )
    );

-- Policy: Authenticated users can send course chat messages
CREATE POLICY "Authenticated users can send course chat messages"
    ON course_chat_messages
    FOR INSERT
    WITH CHECK (
        sender_id = auth.uid()
...
SELECT 
    ...
    'All authenticated users can send and view messages' as access_level;
