-- Fix create_user_account with proper variable casting
...
-- Create function with variable to handle the cast
...
DECLARE
  v_user_id UUID;
  v_role user_role;  -- Declare a variable of type user_role
BEGIN
  ...
  -- Cast the text to user_role enum
  ...
  -- Create profile using the casted variable
  INSERT INTO public.profiles (
    id, 
    first_name, 
    last_name, 
    email, 
    role,           -- This column is type user_role
    status, 
    created_at, 
    updated_at
  ) 
  VALUES (
    ...
    p_last_name, 
    p_email, 
    v_role,         -- Use the casted variable
    'active', 
    NOW(), 
    NOW()
  );
...
-- Test with trainee
SELECT create_user_account('Test', 'Trainee', 'test.trainee.new@example.com', 'Pass123!', 'trainee');

-- Test with instructor
SELECT create_user_account('Test', 'Instructor', 'test.instructor.new@example.com', 'Pass123!', 'instructor');
