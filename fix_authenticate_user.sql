-- Fix authenticate_user to use the correct 'password' column
-- The profiles table uses 'password' not 'password_hash'
...
BEGIN
  ...
  IF v_user.password IS NULL THEN
    RETURN json_build_object('success', FALSE, 'message', 'Account not properly configured. Please contact administrator.');
  END IF;
  ...
  IF v_user.status != 'active' THEN
    RETURN json_build_object('success', FALSE, 'message', 'Account is not active. Please contact administrator.');
  END IF;
