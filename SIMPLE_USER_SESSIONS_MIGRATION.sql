-- Simple user sessions tracking - IP addresses and devices only
-- ADMIN/DEVELOPER VIEW ONLY - No session management
...
DROP FUNCTION IF EXISTS get_user_sessions_admin(UUID, UUID, TEXT, BOOLEAN);
...
-- Function to get user sessions (ADMIN/DEVELOPER ONLY - VIEW ONLY)
CREATE OR REPLACE FUNCTION get_user_sessions_info(
    p_user_id UUID,
...
GRANT EXECUTE ON FUNCTION get_user_sessions_info TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_sessions_info TO anon;

SELECT 'User sessions tracking created! (View-only for Admin/Developer)' as status;