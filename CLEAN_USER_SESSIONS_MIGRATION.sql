-- Clean migration for user sessions tracking system
-- STEP 1: Drop existing functions if they exist
-- STEP 2: Create new restricted functions

-- Drop existing functions (if they exist)
DROP FUNCTION IF EXISTS get_user_sessions(UUID, BOOLEAN);
DROP FUNCTION IF EXISTS get_user_sessions(UUID, UUID, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS end_user_session(UUID, UUID);
DROP FUNCTION IF EXISTS end_user_session(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS end_all_other_sessions(UUID, UUID);
DROP FUNCTION IF EXISTS end_all_other_sessions(UUID, UUID, TEXT);
DROP FUNCTION IF EXISTS record_user_session(UUID, TEXT, TEXT, TEXT, TEXT, TEXT);
...
CREATE OR REPLACE FUNCTION end_user_session_admin(
    p_session_id UUID,
    p_user_id UUID,
    p_requesting_user_role TEXT
)
...
CREATE OR REPLACE FUNCTION end_all_other_sessions_admin(
    p_user_id UUID,
...
GRANT EXECUTE ON FUNCTION get_user_sessions_admin TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_sessions_admin TO anon;
GRANT EXECUTE ON FUNCTION end_user_session_admin TO authenticated;
GRANT EXECUTE ON FUNCTION end_user_session_admin TO anon;
GRANT EXECUTE ON FUNCTION end_all_other_sessions_admin TO authenticated;
GRANT EXECUTE ON FUNCTION end_all_other_sessions_admin TO anon;
